...
> **链表反转**：[lk](https://leetcode.cn/problems/fan-zhuan-lian-biao-lcof/)
>
> ​	迭代版本：同时处理空节点情况
>
> ​	递归版本：注意递归的结束条件是单个节点，同时处理初始情况为空节点
>
> **链表倒数第K个节点（从1开始计数）**[lk](https://leetcode.cn/problems/lian-biao-zhong-dao-shu-di-kge-jie-dian-lcof/)
>
> ​	头节点先走K步，到达正序**第K+1号**节点，同时双指针移动；
...
- **二叉排序树的后序是否合法**
  ...
  > ​	1、「左子树」「右子树」「根节点」
  >
  > ​	2、左子树节点全部小于根节点；右子树节点全部大于根节点
  >
  > ​	3、每一次递归都需要扫描全部节点，时间复杂度O(n^2)
  >
  > **单调栈**：
  >
  > ​	1、。。。。
...
- **二叉树路径求值**
  ...
  > ```
  >
  > **给定目标值：从上至下的路径和为target**
  >
  > ```c++



> **树的子结构：一棵树是否为另一颗树的子结构（空树不是）**[lk](https://leetcode.cn/problems/shu-de-zi-jie-gou-lcof/)
>
> ​	一：先找到根节点匹配的一个节点在开始逐个节点匹配
>
> ​	二：构造调理清晰的递归函数
>
> **节点的最近公共祖先** 
...
​	**数据栈：dataStk**	存储出栈入栈的数据

​	**最小栈：minStk**	维护当前栈内的最小元素（单调递减的栈）
