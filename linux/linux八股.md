...
[文件类型](https://blog.csdn.net/charyman/article/details/123981656)

ls-l 命令可以查看文件属性信息，其中行首的第一个字符即代表该文件的文件类型；

> - 第一列第一个字符为“-”：普通文件；如：-rwxrwxrwx（三个rwx表示文件的权限，可读，可写，可执行；分别是文件所有者对该文件的权限，文件所属组对该文件的权限，其他用户对该文件的权限）
>
>   创建普通文件：`touch newfile`
>
> - 第一列第一个字符为“d”：目录文件；
>
>    创建目录文件：`mkdir directory`
>
> - 第一列第一个字符为“b”：块设备文件
>
> - 第一列第一个字符为“c”：字符设备文件
...
- 同一台主机内
  ...
  - FIFO，命名管道（有名管道）
    ...
    > 支持在无关进程之间交换数据；
    >
    > FIFO有路径名与之关联，在**文件系统**中以一种**特殊文件**形式存在；但是FIFO中的内容却存放在内存中；
    >
